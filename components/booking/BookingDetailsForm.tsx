...
export default function BookingDetailsForm({
  ...
  // Filter out image-type specs, specs not belonging to current service, and entries with empty values
  const serviceDetails = serviceSpecificData
    ? Object.entries(serviceSpecificData).filter(([key, value]) => {
      ...
      // Only include specifications belonging to the current service
    ...
    return (
      <div className="mx-auto mt-6 w-full max-w-5xl">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          {/* ── Left Column: Personal Info & Special Instructions ── */}
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex h-14 items-center gap-3 w-full rounded-xl border border-slate-300 bg-white p-4 text-slate-900">
                ...
                <span className="font-semibold">{name || 'Loading…'}</span>
              </div>
              <p className="mt-1 text-xs text-slate-400">Auto-filled from your account profile</p>
            </div>
            ...
            <div>
              ...
              <div className="flex h-14 items-center gap-3 w-full rounded-xl border border-slate-300 bg-white p-4 text-slate-900">
                ...
                <span className="font-semibold">{phone || 'Loading…'}</span>
              </div>
              <p className="mt-1 text-xs text-slate-400">Auto-filled from your account profile</p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-slate-300 bg-white p-4 text-slate-950 outline-none transition-colors focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
              />
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-6">
              ...
              {service && (
                <div className="mb-4 flex items-center justify-between border-b border-emerald-200/80 pb-3">
                  <span className="text-sm font-medium text-slate-600">Selected Service</span>
                  <span className="text-sm font-bold text-emerald-700">
                    {service} {servicePrice > 0 ? `- ₹${servicePrice}` : ''}
                  </span>
                </div>
              )}
              ...
              {serviceDetails.length > 0 ? (
                <div className="space-y-3">
                  {serviceDetails.map(([key, value]) => {
                    ...
                    return (
                      <div
                        key={key}
                        className="flex items-center justify-between rounded-xl bg-white p-3 shadow-xs border border-emerald-100/80"
                      >
                        <span className="text-sm font-medium text-slate-600">{label}</span>
                        <span className="text-sm font-bold text-emerald-700 text-right max-w-[60%]">
                          {displayValue}
                        </span>
                      </div>
                    );
                ...
                <p className="text-sm text-slate-500">Standard service request (no extra configurations required).</p>
              )}
            </div>
          </div>
        </div>
